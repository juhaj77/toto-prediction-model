...
class MultiHeadAttention extends tf.layers.Layer {
    ...
    constructor(args) {
        ...
        this.dropout = dropout ?? 0.0; // kept for config compatibility; we don't apply dropout at inference
    ...
    build(inputShape) {
        ...
        // Projections implemented with explicit weights to avoid sublayer (Dense) registration issues.
    ...
    call(inputs, kwargs) {
        return tf.tidy(() => {
            ...
            if (this.causal) {
                const mask = tf.linalg.bandPart(tf.ones([seqLen, seqLen]), -1, 0).sub(1).mul(1e9); // -inf on upper triangle
                const bmask = mask.expandDims(0).expandDims(0); // [1,1,s,s]
                logits = logits.add(bmask);
            }
            let weights = tf.softmax(logits, -1);
            // Dropout is intentionally not applied here during inference; if ever enabled, guard with kwargs?.training
            let attn = weights.matMul(v); // [b, h, s, d/h]

            // 4) Merge heads: [b, h, s, d/h] -> [b, s, d]
