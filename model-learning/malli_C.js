const tf = require('@tensorflow/tfjs');
const fs = require('fs');
const csv = require('csv-parser');
const OPETUS_DATA = './Ravit_Opetus_Data_yhdistetty_v2.csv';
const ENNUSTE_DATA = './Ravit_443077258_lahto1.csv';
const MAPPINGS_FILE = './mappings.json';
const MODEL_FOLDER = './ravimalli-mixed'; // Versio 5 uusilla sarakkeilla
const MAX_HISTORY = 8;

// Epoch 32: loss=0.7597 | val_loss=0.5367 | val_acc=0.7242 | LR=0.000075 | Aika: 0m 59s
// Epoch 14: loss=0.7796 | val_loss=0.5314 | val_acc=0.7489 | LR=0.000150 | Aika: 1m 11s

// --- TALLENNUS- JA LATAUSAPUVÄLINEET (BASE64) ---
const saveModelBase64 = async (model) => {
    if (!fs.existsSync(MODEL_FOLDER)) fs.mkdirSync(MODEL_FOLDER);
    await model.save(tf.io.withSaveHandler(async (artifacts) => {
        const weightsBase64 = Buffer.from(artifacts.weightData).toString('base64');
        const modelData = {
            modelTopology: artifacts.modelTopology,
            weightSpecs: artifacts.weightSpecs,
            weightData: weightsBase64
        };
        fs.writeFileSync(`${MODEL_FOLDER}/model_full.json`, JSON.stringify(modelData));
        return { modelArtifactsInfo: { dateSaved: new Date(), modelTopologyType: 'JSON' } };
    }));
};

const loadModelBase64 = async () => {
    const filePath = `${MODEL_FOLDER}/model_full.json`;
    if (!fs.existsSync(filePath)) return null;
    const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    const weightBuffer = Buffer.from(data.weightData, 'base64');
    const arrayBuffer = weightBuffer.buffer.slice(weightBuffer.byteOffset, weightBuffer.byteOffset + weightBuffer.byteLength);
    const artifacts = {
        modelTopology: data.modelTopology,
        weightSpecs: data.weightSpecs,
        weightData: arrayBuffer
    };
    return await tf.loadLayersModel(tf.io.fromMemory(artifacts));
};

// --- APUFUNKTIOT ---

// Radan kunnon muunto 0-1 välille (Unknown = 0.5)
function mapTrackCondition(condition) {
    const c = (condition || "").toLowerCase().trim();
    switch (c) {
        case 'heavy track': return 0.0;
        case 'quite heavy track': return 0.25;
        case 'light track': return 1.0;
        case 'winter track': return 0.75;
        case 'unknown':
        case '':
        default: return 0.5;
    }
}

function getSurname(fullName) {
    if (!fullName) return 'unknown';
    const parts = fullName.trim().toLowerCase().split(' ');
    return parts[parts.length - 1];
}

function pvmToDate(pvmStr) {
    if (!pvmStr || pvmStr.trim() === '' || pvmStr === '0' || pvmStr === 'NaT') return null;
    if (pvmStr.includes('-')) return new Date(pvmStr);
    if (pvmStr.includes('.')) {
        const osat = pvmStr.split('.');
        if (osat.length === 3) {
            const paiva = parseInt(osat[0]);
            const kuukausi = parseInt(osat[1]) - 1;
            let vuosi = parseInt(osat[2]);
            if (vuosi < 100) vuosi += 2000;
            return new Date(vuosi, kuukausi, paiva);
        }
    }
    return null;
}

function kompensoiKmAika(km, matka) {
    if (!km || isNaN(km) || km <= 0) return km;
    const eroMetreina = 2100 - matka;
    const kompensaatio = (eroMetreina / 500) * 1.0;
    return km + kompensaatio;
}

// --- DATAN KÄSITTELY ---

async function lataaJaSiivoaData(tiedostopolku, isTraining = true) {
    let maps = {
        hevoset: {}, valmentajat: {}, ohjastajat: {}, radat: {},
        counts: { h: 1, v: 1, o: 1, r: 1 }
    };

    if (!isTraining && fs.existsSync(MAPPINGS_FILE)) {
        maps = JSON.parse(fs.readFileSync(MAPPINGS_FILE, 'utf8'));
    }

    const getMapID = (map, name, type) => {
        if (!name || name === 'Unknown' || name === '' || name === '0') return 0;
        const cleanName = (type === 'ohjastaja') ? getSurname(name) : name.trim().toLowerCase();
        if (isTraining) {
            if (!map[cleanName]) {
                map[cleanName] = maps.counts[type[0]];
                maps.counts[type[0]]++;
            }
            return map[cleanName];
        }
        return map[cleanName] || 0;
    };

    const rawRows = [];
    return new Promise((resolve, reject) => {
        fs.createReadStream(tiedostopolku)
            .pipe(csv({ separator: ';' }))
            .on('data', (row) => {
                // SUODATUS: Poistetaan poisjääneet (Scratched)
                if (row.Nimi && row.Scratched !== '1') {
                    rawRows.push(row);
                }
            })
            .on('end', async () => {
                const stats = { SH: { en: [], km: [], ke: [], pa: [] }, LV: { en: [], km: [], ke: [], pa: [] } };

                // Tilastojen keruu keskiarvoja varten
                rawRows.forEach(r => {
                    const rotu = r.Is_Suomenhevonen === '1' ? 'SH' : 'LV';
                    const km = kompensoiKmAika(parseFloat(r.Km_aika), parseFloat(r.Matka));
                    if (km > 0) stats[rotu].km.push(km);
                    if (parseFloat(r.Ennatys_nro) > 0) stats[rotu].en.push(parseFloat(r.Ennatys_nro));
                });

                const means = { SH: { en: 28.0, km: 29.0 }, LV: { en: 15.0, km: 16.0 } };
                ['SH', 'LV'].forEach(rotu => {
                    if (stats[rotu].en.length) means[rotu].en = stats[rotu].en.reduce((a,b)=>a+b)/stats[rotu].en.length;
                    if (stats[rotu].km.length) means[rotu].km = stats[rotu].km.reduce((a,b)=>a+b)/stats[rotu].km.length;
                });

                if (isTraining) fs.writeFileSync(MAPPINGS_FILE, JSON.stringify(maps, null, 2));

                // 1. Ryhmittele rivit pelkän RaceID:n mukaan, jotta nähdään ketkä ovat samassa lähdössä
                const lahtoRyhmat = {};
                rawRows.forEach(r => {
                    const lahtoAvain = `${r.RaceID}_${r.Current_Start_Date}`;
                    if (!lahtoRyhmat[lahtoAvain]) lahtoRyhmat[lahtoAvain] = [];

                    // TARKISTUS: Lisätään hevonen ryhmään VAIN jos se ei jo ole siellä.
                    // Tämä poistaa historiarivien moninkertaisen vaikutuksen.
                    const joLisatty = lahtoRyhmat[lahtoAvain].some(h => h.Nimi === r.Nimi);

                    if (!joLisatty) {
                        lahtoRyhmat[lahtoAvain].push(r);
                    }
                });

                // --- Lasketaan kaksi erillistä ranking-mappia: peli% ja voitto% ---
                // Malli saa molemmat + "onko data saatavilla" -bitin erikseen.
                // Näin malli oppii itse painottamaan niitä, eikä ne sekoitu keskenään.
                const peliRankingMap = {};    // ranking peli%:n mukaan (saatavilla 68% lähdöistä)
                const voittoRankingMap = {};  // ranking voitto%:n mukaan (saatavilla 91% lähdöistä)
                const rankingDataMap = {};    // { peliKnown: 0/1, voittoKnown: 0/1 }

                Object.values(lahtoRyhmat).forEach(hevosetLahdossa => {
                    const onkoPeliDataa   = hevosetLahdossa.some(h => parseFloat(h.Peli_pros)   > 0);
                    const onkoVoittoDataa = hevosetLahdossa.some(h => parseFloat(h.Voitto_pros) > 0);

                    // Sortataan VAIN hevoset joilla on aito arvo (> 0)
                    // Näin nollat eivät vääristä muiden järjestystä
                    const peliAidot   = hevosetLahdossa.filter(h => parseFloat(h.Peli_pros)   > 0);
                    const voittoAidot = hevosetLahdossa.filter(h => parseFloat(h.Voitto_pros) > 0);

                    const jarjestysPeli   = [...peliAidot].sort((a, b) =>
                        (parseFloat(b.Peli_pros)   || 0) - (parseFloat(a.Peli_pros)   || 0));
                    const jarjestysVoitto = [...voittoAidot].sort((a, b) =>
                        (parseFloat(b.Voitto_pros) || 0) - (parseFloat(a.Voitto_pros) || 0));

                    hevosetLahdossa.forEach(h => {
                        const avain = `${h.RaceID}_${h.Current_Start_Date}_${h.Nimi}`;
                        const neutraali = (hevosetLahdossa.length + 1) / 2;

                        // Peli%-ranking: vain aidoista arvoista laskettu järjestys
                        const hPeliVal = parseFloat(h.Peli_pros) || 0;
                        const hPeliKnown = (onkoPeliDataa && hPeliVal > 0) ? 1 : 0;
                        peliRankingMap[avain] = hPeliKnown
                            ? jarjestysPeli.findIndex(j => j.Nimi === h.Nimi) + 1
                            : neutraali;

                        // Voitto%-ranking: vain aidoista arvoista laskettu järjestys
                        const hVoittoVal = parseFloat(h.Voitto_pros) || 0;
                        const hVoittoKnown = (onkoVoittoDataa && hVoittoVal > 0) ? 1 : 0;
                        voittoRankingMap[avain] = hVoittoKnown
                            ? jarjestysVoitto.findIndex(j => j.Nimi === h.Nimi) + 1
                            : neutraali;

                        rankingDataMap[avain] = {
                            peliKnown:   hPeliKnown,
                            voittoKnown: hVoittoKnown
                        };
                    });
                });

                const kisaajat = {};
                rawRows.forEach(r => {
                    const id = r.RaceID + '_' + r.Nimi;
                    if (!kisaajat[id]) kisaajat[id] = [];
                    kisaajat[id].push(r);
                });

                const X_hist = []; const X_static = []; const Y = []; const metadata = [];

                Object.values(kisaajat).forEach(hist => {
                    const current = hist[0];

                    // --- Haetaan molemmat rankingt ja "known"-bitit map:eista ---
                    const rankKey = `${current.RaceID}_${current.Current_Start_Date}_${current.Nimi}`;

                    const nHevosia = (lahtoRyhmat[`${current.RaceID}_${current.Current_Start_Date}`] || []).length || 10;
                    const neutraali = (nHevosia + 1) / 2;

                    const peliRankingFinal   = peliRankingMap[rankKey]   ?? neutraali;
                    const voittoRankingFinal = voittoRankingMap[rankKey] ?? neutraali;
                    const rankMeta           = rankingDataMap[rankKey]   ?? { peliKnown: 0, voittoKnown: 0 };

                    const rotu = current.Is_Suomenhevonen === '1' ? 'SH' : 'LV';
                    const cSpec = (current.Current_Special_Cart || "UNKNOWN").toUpperCase();
                    const cSpecActive = cSpec === 'YES' ? 1 : 0;
                    const cSpecKnown = (cSpec === 'YES' || cSpec === 'NO') ? 1 : 0;

                    const etu = (current.Kengat_Etu || "UNKNOWN").toUpperCase();
                    // HUOM: CSV:ssä sarakkeen nimi on "Kengat_Taka" (iso T), ei "Kengat_taka"
                    const taka = (current.Kengat_Taka || current.Kengat_taka || "UNKNOWN").toUpperCase();

                    const etuActive = etu === 'HAS_SHOES' ? 1 : 0;
                    const etuKnown = (etu === 'HAS_SHOES' || etu === 'NO_SHOES') ? 1 : 0;

                    const takaActive = taka === 'HAS_SHOES' ? 1 : 0;
                    const takaKnown = (taka === 'HAS_SHOES' || taka === 'NO_SHOES') ? 1 : 0;

                    // --- STAATTISET FEATURET (Dimension: 25) ---
                    const staticFeats = [
                        (parseFloat(current.Nro) || 1) / 20,          // max 18 datassa
                        getMapID(maps.valmentajat, current.Valmentaja, 'valmentaja') / 2000,
                        (parseFloat(current.Ennatys_nro) || means[rotu].en) / 50,
                        getMapID(maps.ohjastajat, current.Current_Ohjastaja, 'ohjastaja') / 3000,
                        (parseFloat(current.Ika) || 5) / 15,
                        (parseInt(current.Sukupuoli) || 2) / 3,
                        parseInt(current.Is_Suomenhevonen) || 0,

                        etuActive,
                        etuKnown,
                        takaActive,
                        takaKnown,

                        parseInt(current.Kengat_etu_changed) || 0,
                        parseInt(current.Kengat_taka_changed) || 0,

                        (parseFloat(current.Current_Distance) || 2100) / 3100,
                        parseInt(current.Current_Is_Auto) || 0,
                        (parseFloat(current.Peli_pros) || 0) / 100,
                        // Voitto_pros=0 tarkoittaa puuttuvaa dataa (ulkomaiset hevoset), ei oikeaa 0%
                        // Known-bitti kertoo mallille onko tieto luotettava
                        (parseFloat(current.Voitto_pros) || 0) / 100,
                        parseFloat(current.Voitto_pros) > 0 ? 1 : 0,  // voitto_known
                        parseInt(current.Is_Auto_Record) || 0,

                        cSpecActive,
                        cSpecKnown,

                        // Ranking-featuret: hevosen asema suhteessa muihin samassa lähdössä
                        peliRankingFinal / 20,    // sijoitus peli%:n mukaan (1=suosituin)
                        rankMeta.peliKnown,       // 1 jos peli%-data oli saatavilla tässä lähdössä
                        voittoRankingFinal / 20,  // sijoitus voitto%:n mukaan (1=suosituin)
                        rankMeta.voittoKnown      // 1 jos voitto%-data oli saatavilla tässä lähdössä
                    ];

                    const historySeq = hist.slice(0, MAX_HISTORY).map((h) => {
                        const pvm = (h.Hist_PVM || "").toString().trim();
                        const kuski = (h.Ohjastaja || "").toString().trim();

                        if ((pvm === '' || pvm === '0' || pvm === 'NaT') && (kuski === '' || kuski === '0' || kuski === '<undefined>')) {
                            return new Array(25).fill(-1);
                        }

                        const dateNow = pvmToDate(h.Current_Start_Date);
                        const datePrev = pvmToDate(h.Hist_PVM);
                        let daysSince = 30;
                        if (dateNow && datePrev) daysSince = Math.min(365, (dateNow - datePrev) / (1000*60*60*24));

                        const hSpec = (h.Hist_Special_Cart || "UNKNOWN").toUpperCase();
                        const hSpecActive = hSpec === 'YES' ? 1 : 0;
                        const hSpecKnown = (hSpec === 'YES' || hSpec === 'NO') ? 1 : 0;

                        const hEtuStr = (h.Hist_kengat_etu || "UNKNOWN").toUpperCase();
                        const hTakaStr = (h.Hist_kengat_taka || "UNKNOWN").toUpperCase();

                        const hEtuActive = hEtuStr === 'HAS_SHOES' ? 1 : 0;
                        const hEtuKnown = (hEtuStr === 'HAS_SHOES' || hEtuStr === 'NO_SHOES') ? 1 : 0;

                        const hTakaActive = hTakaStr === 'HAS_SHOES' ? 1 : 0;
                        const hTakaKnown = (hTakaStr === 'HAS_SHOES' || hTakaStr === 'NO_SHOES') ? 1 : 0;

                        const rawMatka = parseFloat(h.Matka) || 0;
                        const matkaKnown = rawMatka > 0 ? 1 : 0;
                        const matkaFinal = matkaKnown ? (rawMatka / 3100) : 0.67;

                        let rawKm = parseFloat(h.Km_aika) || 0;
                        const kmKnown = rawKm > 0 ? 1 : 0;
                        const kmFinal = kmKnown ? (rawKm / 100) : (means[rotu].km / 100);

                        let rawPalkinto = parseFloat(h.Palkinto) || 0;
                        const palkintoKnown = rawPalkinto > 0 ? 1 : 0;
                        let palkintoFinal = palkintoKnown ? (Math.log1p(rawPalkinto) / 10) : 0.55;

                        let rawKerroin = parseFloat(h.Kerroin);
                        const kerroinKnown = (!isNaN(rawKerroin) && rawKerroin > 0) ? 1 : 0;
                        let kerroinFinal = kerroinKnown ? (Math.log1p(rawKerroin) / 5) : 0.5;

                        const sijRaw = parseInt(h.Hist_Sij);
                        const sijKnown = (!isNaN(sijRaw) && sijRaw > 0) ? 1 : 0;
                        const sijFinal = sijKnown ? (sijRaw / 20) : 0.5;

                        return [
                            kmFinal,
                            kmKnown,
                            matkaFinal,
                            matkaKnown,
                            daysSince / 365,
                            sijFinal,
                            sijKnown,
                            palkintoFinal,
                            palkintoKnown,
                            kerroinFinal,
                            kerroinKnown,
                            parseInt(h.Hist_Is_Auto) || 0,
                            parseInt(h.Laukka) || 0,
                            (parseInt(h.RataNro) || 1) / 30,  // max 25 datassa
                            getMapID(maps.ohjastajat, h.Ohjastaja, 'ohjastaja') / 3000,
                            getMapID(maps.radat, h.Rata, 'rata') / 500,
                            parseInt(h.Hylatty) || 0,
                            parseInt(h.Keskeytys) || 0,
                            hEtuActive,
                            hEtuKnown,
                            hTakaActive,
                            hTakaKnown,
                            hSpecActive,
                            hSpecKnown,
                            mapTrackCondition(h.Track_Condition)
                        ];
                    });

                    while (historySeq.length < MAX_HISTORY) historySeq.push(new Array(25).fill(-1));

                    X_hist.push(historySeq);
                    X_static.push(staticFeats);

                    if (isTraining) {
                        const sijoitus = parseInt(current.SIJOITUS);
                        Y.push((sijoitus >= 1 && sijoitus <= 3) ? 1 : 0);
                    } else {
                        metadata.push({ Nro: current.Nro, Nimi: current.Nimi, Ohjastaja: current.Current_Ohjastaja });
                    }
                });

                resolve({
                    hist: tf.tensor3d(X_hist),
                    static: tf.tensor2d(X_static),
                    y: isTraining ? tf.tensor2d(Y, [Y.length, 1]) : null,
                    metadata: metadata,
                    histFeatureCount: 25,
                    staticFeatureCount: 25
                });
            });
    });
}

// --- MALLIN RAKENNUS ---
const buildMixedModel = (timeSteps, histFeatures, staticFeatures) => {
    // --- HISTORIA-HAARA (LSTM) ---
    // Pienennetty 128->64 ja 64->32: ~7900 näytteellä 140k parametria oli ylisuuri (17.8x).
    // Nyt ~42k parametria = 5.4x suhde, mikä sopii tähän datasetiin.
    const historyInput = tf.input({ shape: [timeSteps, histFeatures], name: 'history_input' });

    let maskedInput = tf.layers.masking({ maskValue: -1 }).apply(historyInput);

    let lstm1 = tf.layers.lstm({
        units: 64,
        returnSequences: true,
        recurrentDropout: 0.1,          // Regularisoi LSTM:n sisäistä tilaa
        kernelRegularizer: tf.regularizers.l2({ l2: 0.001 })
    }).apply(maskedInput);

    let lstm2 = tf.layers.lstm({
        units: 32,
        returnSequences: false,
        recurrentDropout: 0.1,
        kernelRegularizer: tf.regularizers.l2({ l2: 0.001 })
    }).apply(lstm1);

    // BatchNorm LSTM:n jälkeen ennen dropoutia - stabiloi gradientteja
    let histBn = tf.layers.batchNormalization().apply(lstm2);
    let histDrop = tf.layers.dropout({ rate: 0.3 }).apply(histBn);

    // --- STAATTINEN HAARA ---
    // 25 featurea -> 48 -> 32: tasapainoisempi supistus
    const staticInput = tf.input({ shape: [staticFeatures], name: 'static_input' });
    let static1 = tf.layers.dense({
        units: 48,
        activation: 'relu',
        kernelRegularizer: tf.regularizers.l2({ l2: 0.001 })
    }).apply(staticInput);
    let staticBn = tf.layers.batchNormalization().apply(static1);
    let static2 = tf.layers.dense({ units: 32, activation: 'relu' }).apply(staticBn);
    let staticDrop = tf.layers.dropout({ rate: 0.3 }).apply(static2);  // sama kuin hist-haara

    // --- YHDISTÄMINEN ---
    // 32 + 32 = 64 -> 48 -> 24 -> 1
    const concatenated = tf.layers.concatenate().apply([histDrop, staticDrop]);

    let dense1 = tf.layers.dense({ units: 48, activation: 'relu' }).apply(concatenated);
    dense1 = tf.layers.batchNormalization().apply(dense1);
    dense1 = tf.layers.dropout({ rate: 0.25 }).apply(dense1);

    let dense2 = tf.layers.dense({ units: 24, activation: 'relu' }).apply(dense1);

    const output = tf.layers.dense({ units: 1, activation: 'sigmoid' }).apply(dense2);

    const model = tf.model({ inputs: [historyInput, staticInput], outputs: output });

    model.compile({
        optimizer: tf.train.adam(0.0003),  // Hieman korkeampi LR pienemmälle mallille
        loss: 'binaryCrossentropy',
        metrics: ['accuracy']
    });

    return model;
};

// --- PÄÄTOIMINNOT ---

async function ajaOpetus() {
    console.log('--- ALOITETAAN OPETUS (Kuski-uskollisuus & aikaleimat) ---');
    const data = await lataaJaSiivoaData(OPETUS_DATA, true);
    const model = buildMixedModel(MAX_HISTORY, data.histFeatureCount, data.staticFeatureCount);

    let bestLoss = Infinity;
    let patienceCounter = 0;
    let lrPatienceCounter = 0;
    const EARLY_STOP_PATIENCE = 16;
    const LR_REDUCE_PATIENCE = 5;

    let epochStartTime;

    await model.fit([data.hist, data.static], data.y, {
        epochs: 50,
        batchSize: 64,
        validationSplit: 0.1,
        shuffle: true,
        classWeight: { 0: 1.0, 1: 2.0 },
        callbacks: {
            onEpochBegin: async () => {
                epochStartTime = Date.now();
            },
            onEpochEnd: async (epoch, logs) => {
                const durationMs = Date.now() - epochStartTime;
                const minutes = Math.floor(durationMs / 60000);
                const seconds = ((durationMs % 60000) / 1000).toFixed(0);
                const timeStr = `${minutes}m ${seconds}s`;

                const vLoss = logs.val_loss;
                const vAcc = (logs.val_acc || logs.val_accuracy || 0).toFixed(4);
                let currentLR = model.optimizer.learningRate;

                console.log(`Epoch ${epoch + 1}: loss=${logs.loss.toFixed(4)} | val_loss=${vLoss.toFixed(4)} | val_acc=${vAcc} | LR=${currentLR.toFixed(6)} | Aika: ${timeStr}`);

                if (vLoss < bestLoss) {
                    bestLoss = vLoss;
                    patienceCounter = 0;
                    lrPatienceCounter = 0;
                    console.log(`   ⭐ Uusi paras val_loss, tallennetaan...`);
                    await saveModelBase64(model);
                } else {
                    patienceCounter++;
                    lrPatienceCounter++;
                    if (lrPatienceCounter >= LR_REDUCE_PATIENCE) {
                        const newLR = currentLR * 0.5;
                        model.optimizer.learningRate = newLR;
                        console.log(`   📉 Lasketaan oppimisnopeutta: ${newLR.toFixed(6)}`);
                        lrPatienceCounter = 0;
                    }
                    if (patienceCounter >= EARLY_STOP_PATIENCE) {
                        console.log(`\n--- Early Stopping ---`);
                        model.stopTraining = true;
                    }
                }
            }
        }
    });
}

async function ajaEnnustus() {
    console.log('\n--- ALOITETAAN ENNUSTUS ---');
    const model = await loadModelBase64();
    if (!model) {
        console.log("Malli-tiedostoa ei löytynyt. Aja opetus ensin.");
        return;
    }

    const data = await lataaJaSiivoaData(ENNUSTE_DATA, false);
    if (data.hist.shape[0] === 0) {
        console.log("Ei ennustettavaa dataa löytynyt.");
        return;
    }

    const predictions = model.predict([data.hist, data.static]);
    const scores = await predictions.data();

    const tulokset = data.metadata.map((m, i) => {
        const prob = scores[i];
        const rajakerroin = (1 / prob).toFixed(2);
        return {
            Nro: m.Nro,
            Hevonen: m.Nimi,
            Ohjastaja: m.Ohjastaja,
            Todennakoisyys: (prob * 100).toFixed(1) + '%',
            Malli_Kerroin: rajakerroin,
            Arvio: prob > 0.5 ? 'PELATTAVA' : 'HUTI'
        }
    }).sort((a, b) => parseFloat(b.Todennakoisyys) - parseFloat(a.Todennakoisyys));

    console.table(tulokset);
    data.hist.dispose();
    data.static.dispose();
    predictions.dispose();
}
/*
(async () => {
    await ajaOpetus();
    await ajaEnnustus();
})();
 */

ajaEnnustus();
