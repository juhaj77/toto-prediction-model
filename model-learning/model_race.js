// ═══════════════════════════════════════════════════════════════════════════════
// TOTO PREDICTION MODEL — ravimalli-race.js
//
// Reads: Learning_Data.json  (training)
// Writes: ravimalli-race/model_full.json  (trained model weights)
...
function recallAtThree(yTrue, yPred) {
    return tf.tidy(() => {
        ...
        // True positives captured in top-3
        const captured = yT.mul(top3Mask).sum(1);  // [b]

        // Denominator = number of actual positives per race (avoid div by zero)
        const denom = yT.sum(1);                   // [b]
...
const MODEL_FOLDER  = './ravimalli-race';
const MAX_HISTORY   = 8;
...
// ─── FEATURE HELPERS ──────────────────────────────────────────────────────────
// Duplicated from ravimalli.js to keep this file self-contained.
    ...
    await model.fit([histTrain, statTrain, maskTrain], yTrain, {
        ...
        callbacks: {
            ...
            onEpochEnd: async (epoch, logs) => {
                ...
                // Compute manual ROC-AUC on full validation set by filtering real runner slots (mask==1)
